...
enum GeoProjection {
    ...
    static let lonMax: Double = 220     // was -33 → 180 → 220. Spans the Eastern
                                        // Hemisphere (Europe/Africa/Asia/Australia)
                                        // and now far enough east to fit French
                                        // Polynesia (Tahiti, real lon −149.6, stored
                                        // as +210.4 to cross the antimeridian and sit
                                        // near Fiji instead of wrapping to the far
                                        // west). unit() doesn't use lonMax, so
                                        // existing points are unaffected; only the
                                        // canvas grows east.
